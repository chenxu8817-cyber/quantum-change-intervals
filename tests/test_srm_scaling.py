...
class ResourceGuardTests(unittest.TestCase):
    def test_resource_estimate_uses_binomial_candidate_count(self) -> None:
        estimate = estimate_dense_resources(50, 1)
        self.assertEqual(estimate["candidate_count"], math.comb(51, 2))
        self.assertEqual(
            estimate["gram_bytes"],
            8 * math.comb(51, 2) ** 2,
        )

    def test_dense_case_is_rejected_before_allocation(self) -> None:
        with self.assertRaisesRegex(MemoryError, "dense Gram estimate"):
            run_exact_case(
                n=50,
                interval_count=1,
                overlap=0.8,
                max_gram_bytes=1_000_000,
            )
