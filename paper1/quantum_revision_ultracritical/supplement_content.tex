...
Section~\ref{supp:dual-certificates}. Section~\ref{supp:matching} extends
these results to moving overlaps through weighted hull compression and
adaptive endpoint blocks. Section~\ref{supp:direct-srm} derives a labelwise
local diagonal law from the retained-orthogonality, incidence-trace, and
odd-cosine estimates used in the global analysis.
...
To apply the inner and outer theorems along a sparse subsequence, define
\(\lambda_n=1/\log n\) at omitted dimensions in the inner case and
\(\lambda_n=\sqrt n\) in the outer case. This extension preserves the selected
Gram matrices and satisfies the corresponding regime assumptions. The
continuum case requires no extension because
