...
For a generic overlap \(0\le c<1\), we shall also use
...
The common asymptotic success laws follow from the sectorwise dual certificates
in Section~\ref{supp:dual-certificates} and the global trace theorem in
Section~\ref{supp:trace-laws}. Section~\ref{supp:direct-srm} proves the stronger
labelwise square-root-diagonal law; only its retained-orthogonality lemma is
reused in the global trace argument. Section~\ref{supp:matching} extends the
analysis to moving overlaps through weighted hull compression and adaptive
endpoint blocks.
...
has the strict cardinality \(n(n-1)/2\).  If

\[
 [V_n(c)]_{a,u}=c^{u-a}\mathbf1_{\{a\le u\}},
\]

if \(R_{\mathcal I}\) restricts ordered row pairs to \(a\le b\), and if
\(C_{<}\) includes strict column pairs \(u<v\), then
...
where \(B_{1,n}\) is the interval-site incidence matrix. This retains the
physical length factor \(D_n\) while replacing the strict-hull depth weights
by one. Then
...
Uniformly in \(k\),
\(\csc(\theta_k/2)=2(2m+1)/[\pi(2k-1)]+O(1)\).  The identity
...
together with summation by parts gives \eqref{eq:supp-leverage-profile},
including \(j=1,m\).  Summing the singular values gives
...
Since \(\sum_k\csc^2(\theta_k/2)=O(m^2)\), equations
\eqref{eq:supp-cm-diagonal-expansion}--\eqref{eq:supp-odd-cosine-gram}
show that the empirical variance of the diagonal is bounded.  Cauchy--
Schwarz proves \eqref{eq:supp-cm-flatness}.
\end{proof}
...
\begin{lemma}[Singular-safe weighted block--tail extension]
...
the stated direct branch follows.  In particular, no positive \(L\), however
small, is sent to the zero branch.

The suffix identity is checked coordinatewise.  For \(k\ge B+1\), its tail
term is
\(s(c)c^{B+1-a}c^{k-B-1}=s(c)c^{k-a}\), and the block-supported
coordinates agree by definition.  Moreover,
...
dominates every block-supported physical suffix.  For all sufficiently large
\(m\), it has the explicit upper budget
...
\]
The same conclusion holds uniformly for triangular families whenever all four
displayed limiting conditions hold uniformly over the auxiliary index.
...
\(r_{m,j}=\mathbf1_{\{j,\ldots,m\}}\).  The block-supported physical
suffix is
...
This proves the displayed budget.  The exact ratio is
...
No monotonicity of \(c_j\) is used.  The endpoint constructions contain no
inverse power of \(c\).
\end{proof}

\begin{corollary}[Weighted endpoint-cell certificate]
\label{cor:supp-weighted-cell-certificate}
Partition the sites into consecutive endpoint blocks \(E_1,\ldots,E_B\).
...
Compression preserves this order.  Every interval has its endpoints in a
unique cell \(p\le q\); the exact hull identity then proves feasibility, and
adding the remaining positive cells cannot weaken it.  The diagonal cells
...
\subsection{Tensorization, triangular compression, and the sector optimum}
...
\begin{theorem}[Quantitative sector-oracle upper bound]
...
Combining Theorems~\ref{thm:supp-global-trace} and
\ref{thm:supp-sector-upper} with
\(P_{\rm tr}\le P_{\rm SRM}\le P_{\rm opt}\) proves the common rate on compact
parameter windows. The lower estimate comes from retained rectangular
pinching, while the upper estimate comes from dual feasibility.

\section{Local SRM refinement and Kac--Murdock--Szeg{\H o} uniform integrability}
\label{supp:direct-srm}

This section proves the compact-uniform local statement quoted in the main
text. It strengthens the common success probability law by resolving the SRM
contribution label by label. The retained-orthogonality lemma below is also
used in the global trace argument.

\subsection{One- and two-excitation local laws}

We first record the finite-codimension estimate used when the three
excitation strata are made row-orthogonal.  Let
...
Expanding the product of sine squares expresses the bracket in
\eqref{eq:supp-b1-exact-diagonal} as
...
Products therefore make the right-hand side of
\eqref{eq:supp-b2-square-majorant} converge to \(1/\pi^2\) in empirical
\(L^2\) over the triangle.  Proposition~\ref{prop:supp-b2-trace} says the
left-hand side has the same empirical mean.  The normalized deficit is
nonnegative and uniformly bounded, hence converges to zero in empirical
\(L^2\).  This proves \eqref{eq:supp-b2-local}.
\end{proof}
...
with \(\theta\) selected by the second endpoint equation.  Every nonzero
...
\subsection{Retained orthogonality and completion of the local law}
...
the compact-uniform statements
...
monotonicity and the trace laws give
...
\begin{theorem}[Compact-uniform physical local law]
...
The Kac--Murdock--Szeg{\H o} truncation yields the local law uniformly on
compact \(t\)-windows through the sequential compactness criterion; this local
law is qualitative. The global trace and dual bounds in
Sections~\ref{supp:trace-laws} and \ref{supp:dual-certificates} instead give the
explicit rate \(O_T(\log\log n/\log n)\) for the three success criteria.

\section{Uniform matching and hull dominance}
\label{supp:matching}

We next connect the compact critical window to the large-\(\tau\) regime at
leading relative order.
...
\eqref{eq:supp-inner-assumptions}.  Equation
\eqref{eq:supp-outer-formula} is a comparison of asymptotic formulas; it is
not obtained by inserting a moving \(c_n\) into the pointwise fixed-overlap
theorem.

\subsection{Weighted continuum interface and hull dominance}

When \(\lambda_n=n(1-c_n)\) stays away from zero, the physical length and hull
weights are no longer close to the identity. The following argument therefore
retains these weights within each block through the exact hull coordinates,
local block-and-tail certificates, and matched rectangular pinching, instead
of using \eqref{eq:supp-inner-schatten-sandwich}.
...
block pinching is Schatten--1 contractive, and compression to
...
resulting inequality by two proves the claim.  This argument is specific to
  matched row-column blocks; arbitrary entry deletion need not be contractive.
\end{proof}
...
\lceil n/B\rceil\}\).  The elliptic endpoint expansion gives, uniformly
for \(\lambda\in K\),
...
\[
 ...
 \min_pm_p\to\infty,\quad
 \max_pm_p(1-c)\to0,\quad
...
The strict triangle supplies the exact identity
...
away from zero on \(K\).  This proves the scaled formulation without
dividing through a vanishing compact endpoint.
\end{proof}

The following table summarizes the normalization conventions used here and
below.
...
implies, with the ceiling retained,
...
It remains to match logarithms.  The endpoint expansion is
...
\(\log m_n/\ell_n\to1\).  The two index sets cover an arbitrarily
...
\begin{proof}
The exact identity in the preceding lemma gives \(h_n\to\infty\), so use its
adaptive balanced blocks.  For either possible block size
\(m_{p,n}\in\{m_n,m_n+1\}\),

\[
 m_{p,n}(1-c_n)\to0,\quad
 \frac{\log m_{p,n}}{\ell_n}\to1,\quad
 m_{p,n}p_1(c_n)\to\infty.
\]

The same weighted similarity, matched pinching, and strict-cell count as in
Theorem~\ref{thm:supp-continuum-interface} give
...
and hence \(P_{\rm tr}(G_n)\ge p_1(c_n)^2[1-o(1)]\).
Proposition~\ref{prop:supp-weighted-volterra-certificate} and
Lemma~\ref{lem:supp-weighted-block-tail} give one-sided certificate traces
\(T_{p,n}=m_{p,n}p_1(c_n)[1+o(1)]\), uniformly in \(p\).  The cell
corollary, including every diagonal and terminal cell, then gives

\[
 \operatorname{tr}\mathcal K_{\ge2,n}
 \le M_np_1(c_n)^2[1+o(1)].
\]
...
Sector Minkowski and the usual trace--SRM--optimum ordering complete the
squeeze.

The proof is quantitative. One sufficient relative error bound for the
...
The terms arise, respectively, from local diagonal similarity, the Volterra
boundary repair, logarithmic matching, the common rank-one tail, diagonal
cells, one excitation, and the vacuum. Lemma
\ref{lem:supp-adaptive-block-parameters} and the scalar estimate above make
every term vanish.
\end{proof}
...
For literal application of sequence-wise inner and outer theorems on sparse
dimension subsequences, fill omitted inner dimensions with
\(\lambda_n=1/\log n\), and omitted outer dimensions with
\(\lambda_n=\sqrt n\).  The selected Gram matrices are unchanged and the
filled sequences obey the required regime hypotheses.  The compact theorem
is already uniform and needs no filling.

Thus every subsequence has a further subsequence on which the displayed
ratio tends to one.  The standard subsequence criterion proves the full
limit.  The argument uses neither monotonicity nor regular variation.
\end{proof}

The following table summarizes the regimes, their proof inputs, and their
common limits.
...
Let \(B_{1,n}\) be the interval--site incidence matrix.  A direct count gives
...
More quantitatively, partition the sites into \(R\) balanced consecutive
blocks of sizes \(\ell_p\), retain only matched row--column cells with
\(p<r\), and project each one-dimensional column factor off the constant
and incidence-sum directions used in
Lemma~\ref{lem:supp-retained-orthogonality}.  The resulting rectangular
pinching \(Z_{n,R}^{\rm ret}\)
satisfies
...
For the full upper bound, fix \(R\) and decompose both endpoint triangles
...
\]
Thus, after summing the finitely many macroblocks for fixed \(R\), the one-
and zero-match blocks are
\(o(n^2\log^2n)\) for fixed \(R\).  The diagonal cells are rescaled copies of
\(Z_{n/R}\).  Before taking a limsup, note that, up to the fixed reversal of
the right-endpoint coordinate,
\[
 Z_n=P_{\Lambda_{n-1}}
...
To restore the physical weights, note that
...
\(O_T((\log n)^{-2})\).  The trace norm is Lipschitz under these state-matrix
perturbations, so the physical matrix obeys
\eqref{eq:supp-global-trace-rate}.

For the matching upper limit, apply the Schatten--1 triangle inequality to
...
The numerical checks in this section are diagnostics, not inputs to any
proof.  For each \((n,\tau)\), we set
\(c=1-\tau/[n(\log n)^2]\), form the full dense physical Gram matrix, and
diagonalize it.  Table~\ref{tab:supp-critical-diagnostics} reports the scaled
...
consistent with the logarithmic theorem and explains why a finite-size
polar/SRM gap can initially appear to grow.
...
Both signs occur in these finite-precision calculations, so the computed
diagonals are inconsistent with coordinatewise square-root additivity. In the
same runs, the minimum eigenvalue of \(\widehat G_n-K_n\) lay between
\(-2.9\times10^{-15}\) and \(-2.2\times10^{-15}\), consistent with positive
semidefiniteness at the working precision; the retained row-orthogonality
residuals were below \(1.8\times10^{-13}\).

The complete twelve-case CSV, from which the displayed rows are selected, is
reproduced by

\begin{verbatim}
python code/unknown_length_full_srm_probe.py \
  --n 12 18 24 32 --tau 0.25 1 4 --retained \
  --output data/critical_srm_diagnostics.csv
\end{verbatim}

The machine-readable output used for the displayed rows is included as
\path{data/critical_srm_diagnostics.csv}.
