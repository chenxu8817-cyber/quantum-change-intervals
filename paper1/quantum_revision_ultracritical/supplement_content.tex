...
labelwise local law for the square-root diagonal; only its retained-orthogonality lemma is
...
The endpoint constructions contain no inverse power of \(c\).
\end{proof}
...
\section{Local SRM refinement}
\label{supp:direct-srm}

The local result below strengthens the common success probability law by
resolving the SRM contribution label by label.
...
The remainder of this section proves the theorem. Its
retained-orthogonality lemma is also used in the global trace argument.
...
the statements that are uniform on compact parameter intervals
...
This completes the proof of Theorem~\ref{thm:supp-full-local-law}. The local
law is qualitative; the global trace and dual bounds in
Sections~\ref{supp:trace-laws} and \ref{supp:dual-certificates} give the
...
Sector Minkowski and
\(P_{\rm tr}\le P_{\rm SRM}\le P_{\rm opt}\) complete the squeeze.
...
cells, one excitation, and the vacuum.
Lemma~\ref{lem:supp-adaptive-block-parameters} and the two logarithmic
estimates preceding the error bound make every term vanish.
\end{proof}
...
Let \(B_{1,n}\) be the incidence matrix between intervals and sites. A direct count gives
...
blocks of sizes \(\ell_p\), retain only matched cells of row and column blocks with
...
with one or zero matching endpoint indices are
...
\(P_{\rm tr}\le P_{\rm SRM}\le P_{\rm opt}\) gives the common rate on compact
parameter windows. Retained rectangular pinching supplies the lower estimate,
and dual feasibility supplies the upper estimate.
