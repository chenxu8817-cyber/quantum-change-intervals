...
The common success law and its rate follow from the global trace theorem in
Section~\ref{supp:trace-laws} and the sectorwise dual certificates in
Section~\ref{supp:dual-certificates}. Section~\ref{supp:matching} then extends
the analysis to moving overlaps through weighted hull compression and adaptive
endpoint blocks. After this main proof chain, Section~\ref{supp:direct-srm}
gives a downstream labelwise local diagonal refinement. It reuses retained
orthogonality, incidence-trace laws, and odd-cosine estimates from the global
analysis, but it is not an input to the trace and sectorwise squeeze that
proves the common-limit theorem.
...
The following construction is shared by the global trace argument and the
downstream labelwise local refinement. It is stated here before either use.
...
blocks of sizes \(\ell_p\). Retain only matched row and column cells with
...
The first three terms measure the local similarity, Volterra repair, and
logarithmic matching errors. The next two account for the common rank-one tail
and the diagonal cells, while the final two control the one-excitation and
vacuum sectors.
...
To apply the sequence-wise inner and outer theorems to a sparse subsequence of
dimensions, extend the parameters to omitted dimensions using
\(\lambda_n=1/\log n\) in the inner case and \(\lambda_n=\sqrt n\) in the
outer case. The extension leaves the selected Gram matrices unchanged and
satisfies the corresponding regime assumptions. The compact theorem is
already uniform.
...
This completes the proof of Theorem~\ref{thm:supp-full-local-law}. The global
trace and dual bounds in
...
sandwich is valid in exact arithmetic whenever the postprocessed matrices are
feasible, while the stored residuals quantify the numerical recheck at the
working precision.
...
Both signs occur, ruling out coordinatewise square-root additivity for these
finite-precision diagonals. The minimum eigenvalue of \(\widehat G_n-K_n\) lay
